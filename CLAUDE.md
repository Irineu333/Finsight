...
- **Chart of accounts:** every account and card is an `Account` with a `type` from the **closed** set `{ASSET, LIABILITY, INCOME, EXPENSE, EQUITY, CONVERSION}` (`core/ledger`). A card is a facade linked to its `Account` by `accountId`, and reads closure **and currency** from it — no copy. `CONVERSION` is where the residue of a cross-currency operation lands; it has a type of its own rather than reusing `EQUITY`, which means exactly "adjustment" to the derived label and to the editability gates. A **category is not in the chart**: it is a dimension (see below), so it owns its own `isArchived`. Beyond the user's accounts and cards the chart holds four *kinds* of system row — the two nominals every expense and income lands on, reconciliation, and conversion — and they exist **one per currency**, resolved by `(type, name, currency)`.
...
- **Reads:** every figure — balance, opening balance, invoice owed, category spending, net worth — is `Σ entries`, via `IEntryRepository`. There is no per-type sign rule and no second way to compute a number. Every read that does not filter to a single account answers **per currency**, and no aggregation ever sums two of them; reducing that to one figure is the consolidation layer's job (`ConsolidateFigureUseCase`, `:core:model`), outside the ledger.
...
- **Writes:** callers express **intent** by identity — `TransactionLeg(type, amount, accountId, dimensionId?)` plus a `ContraLeg(nature, dimensionId?)` for a one-sided intent. A leg carries **no currency**: it is read from the account it lands on, so posting an amount in the wrong currency is inexpressible rather than refused. Resolving a facade to an id is the caller's job; completing and balancing the intent is the writer's, because it creates the system account on demand — and, when an operation spans currencies, lands each currency's residue on its `CONVERSION` account, undimensioned.
