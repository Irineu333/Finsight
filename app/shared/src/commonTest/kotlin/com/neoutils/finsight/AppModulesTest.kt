...
class AppModulesTest {
    ...
    fun navCatalogProjectionsAreConsistent() {
        ...
        // The desktop rail excludes mobile-only destinations (Support).
        val railCount = destinations.count { !it.mobileOnly }
        assertEquals(destinations.size - destinations.count { it.mobileOnly }, railCount)
        assertTrue(destinations.any { it.mobileOnly })

        // The mobile grid (non-primary) includes the mobile-only destination.
        assertTrue(destinations.filter { !it.primaryTab }.any { it.mobileOnly })
