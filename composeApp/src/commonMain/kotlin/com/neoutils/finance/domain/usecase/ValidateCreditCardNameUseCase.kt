...
class ValidateCreditCardNameUseCase(
    ...
    suspend operator fun invoke(name: String): UiText? {
        if (name.isBlank()) {
    ...
    private suspend fun hasDuplicateName(name: String): Boolean {
        val creditCards = repository.getAllCreditCards()
        return creditCards.any { it.name.equals(name.trim(), ignoreCase = true) }
    }
}
