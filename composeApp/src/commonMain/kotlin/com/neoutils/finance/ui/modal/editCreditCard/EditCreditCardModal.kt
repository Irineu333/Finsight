...
class EditCreditCardModal(private val creditCardId: Long) : ModalBottomSheet() {

        @Composable
        override fun ColumnScope.BottomSheetContent() {
                val viewModel =
                        koinViewModel<EditCreditCardViewModel>(key = key) {
                ...
                LaunchedEffect(
                        ...
                        closingDay.edit {
                                replace(0, length, uiState.currentClosingDay?.toString() ?: "")
                        }
                }
        ...
        private fun parseMoneyToDouble(formatted: String): Double {
                val digitsOnly =
                        formatted.replace("R$", "").replace(".", "").replace(",", ".").trim()
