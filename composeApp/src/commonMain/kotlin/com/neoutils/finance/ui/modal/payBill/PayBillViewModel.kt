...
class PayBillViewModel(
    ...
    fun payBill(amount: Double, date: LocalDate) {
        viewModelScope.launch {
            // 1. Criar transação de pagamento (apenas se valor > 0)
            if (amount > 0) {
                payBillUseCase(invoiceId = invoiceId, amount = amount, date = date)
            }

            // 2. Marcar fatura como PAGA
            val paidAt = date.atStartOfDayIn(TimeZone.currentSystemDefault()).toEpochMilliseconds()
