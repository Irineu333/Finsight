...
class CreditCardsViewModel(
    ...
    val uiState = creditCardRepository.observeAllCreditCards().map { creditCards ->
        CreditCardsUiState(
            creditCards = creditCards.map { creditCard ->
                val invoice = invoiceRepository.getLatestInvoice(creditCard.id)

                CreditCardUi(
                    creditCard = creditCard,
                    invoiceUi = invoice?.let {
                        invoiceUiMapper.toUi(invoice)
                    }
                )
            }
        )
