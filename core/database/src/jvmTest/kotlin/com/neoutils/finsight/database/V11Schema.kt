 ...
 * fixture that is not the real old schema proves nothing about the real migration.
 *
 * It is [V10_SCHEMA] plus exactly what `10 → 11` adds: `accounts.yieldsInterest`, and a
 * `categories.systemKey` unique across the table.
 *
 * Order matters only in that foreign keys are not enforced at CREATE time.
 */
internal val V11_SCHEMA: List<String> = listOf(
