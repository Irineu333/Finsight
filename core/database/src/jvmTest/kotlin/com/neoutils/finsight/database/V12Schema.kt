...
/**
 * The v12 schema — the shape a device is on before the upgrade to 13. Frozen history, in
 * the mould of [V7_SCHEMA], [V10_SCHEMA] and [V11_SCHEMA]: a fixture that is not the real
 * old schema proves nothing about the real migration.
 *
 ...
 * [Migration12To13] exists to make explicit.
 *
 * Order matters only in that foreign keys are not enforced at CREATE time.
 */
internal val V12_SCHEMA: List<String> = listOf(
    ...
    "CREATE TABLE IF NOT EXISTS `categories` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `name` TEXT NOT NULL, `iconKey` TEXT NOT NULL, `type` TEXT NOT NULL, `createdAt` INTEGER NOT NULL, `dimensionId` INTEGER NOT NULL, `isArchived` INTEGER NOT NULL, `systemKey` TEXT, FOREIGN KEY(`dimensionId`) REFERENCES `dimensions`(`id`) ON UPDATE NO ACTION ON DELETE NO ACTION )",
    ...
    "CREATE TABLE IF NOT EXISTS `accounts` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `name` TEXT NOT NULL, `type` TEXT NOT NULL, `currency` TEXT NOT NULL, `iconKey` TEXT NOT NULL, `isDefault` INTEGER NOT NULL, `createdAt` INTEGER NOT NULL, `isArchived` INTEGER NOT NULL, `yieldsInterest` INTEGER NOT NULL)",
    ...
    "CREATE TABLE IF NOT EXISTS `budgets` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `iconCategoryId` INTEGER NOT NULL, `iconKey` TEXT NOT NULL, `title` TEXT NOT NULL, `amount` REAL NOT NULL, `currency` TEXT NOT NULL, `period` TEXT NOT NULL, `limitType` TEXT NOT NULL, `percentage` REAL, `recurringId` INTEGER, `createdAt` INTEGER NOT NULL)",
    ...
    "CREATE TABLE IF NOT EXISTS `exchange_rates` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `currency` TEXT NOT NULL, `date` TEXT NOT NULL, `rate` REAL NOT NULL, `source` TEXT NOT NULL)",
    "CREATE UNIQUE INDEX IF NOT EXISTS `index_exchange_rates_currency_date_source` ON `exchange_rates` (`currency`, `date`, `source`)",
    "CREATE INDEX IF NOT EXISTS `index_exchange_rates_currency_date` ON `exchange_rates` (`currency`, `date`)",
)
