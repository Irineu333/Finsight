 ...
 * `CloseAccountUseCase`'s — so a screen that guesses wrong still gets the right
...
class RetireActionTest {
    ...
    fun `movement makes it a closure, its absence a deletion`() {
        assertEquals(RetireAction.CLOSE, retireActionOf(hasMovement = true))
    ...
    fun `the ui models expose the same rule, so the two screens cannot drift`() {
        ...
        assertEquals(RetireAction.CLOSE, moved.retireAction)
