...
| `create_recurring` | `type`, `amount`, `day_of_month`, `title?`, `category_id?`, `account_id?` \| `card_id?` | ⬆ `SaveRecurringUseCase(id = 0, …)` | passou a devolver o `Recurring` gravado — sem isso não haveria o que responder nem o que referenciar no registro. O que `RecurringForm.toRecurring` descartaria em silêncio é recusado antes do formulário: `card_id` numa receita, nomeando o cartão e não a conta que ninguém deu, e uma categoria que a direção não classifica |
| `update_recurring` | `id`, `type?`, `amount?`, `day_of_month?`, `title?`, `category_id?`, `account_id?`, `card_id?` | ⬆ `SaveRecurringUseCase(id, …)` | mesmo use case, id não-zero. As duas mesmas recusas, e a do cartão vale tanto para o citado em `card_id` quanto para aquele em que o template já posta; a da categoria, tanto para a citada em `category_id` quanto para **a que o template já tem**, que a chamada não mencionou. `category_id = 0` é como a chamada diz *sem categoria*, e a única forma de a edição tirar uma classificação. Um template cuja categoria era o seu único nome é recusado por ficar sem nenhum: `TITLE_OR_CATEGORY_REQUIRED`, com o título junto como saída. Dar `account_id` e `card_id` na mesma chamada também é recusado, em vez de a conta vencer em silêncio |
| `delete_recurring` *(Apagar)* | `id` | ⬆ `DeleteRecurringUseCase` | consulta ⬆ `ResolveRecurringRetirabilityUseCase` |
| `create_installment` | `card_id`, `amount`, `count`, `date?`, `title?`, `category_id?`, `invoice_month?` | ✔ `AddInstallmentUseCase(form, installments)` | as N transações são all-or-nothing, garantia do repositório. Um parcelamento é sempre despesa, e a direção não é parâmetro: uma categoria de receita em `category_id` é recusada antes do formulário, nomeando a categoria e a direção, em vez de ser descartada em silêncio por `TransactionForm.from` |
