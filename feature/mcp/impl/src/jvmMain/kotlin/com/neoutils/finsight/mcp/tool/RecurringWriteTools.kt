...
internal class CreateRecurringTool(
    ...
    override val description: String =
        "Create a monthly template — something that repeats on the same day every month, like " +
            ...
            "Give exactly one of account_id or card_id; an income always goes to an account. " +
...
internal class UpdateRecurringTool(
    ...
    override suspend fun call(arguments: JsonObject?) = writing {
        ...
        saveRecurring(
            id = id,
            type = arguments.oneOf("type", RECURRING_TYPES.keys.toList())
            ...
            category = arguments.long("category_id")?.let { categoryRepository.require(it) }
                ?: stored.category,
            account = account,
            creditCard = card,
