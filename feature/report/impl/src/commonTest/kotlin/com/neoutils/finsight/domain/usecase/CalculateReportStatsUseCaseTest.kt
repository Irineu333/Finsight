...
class CalculateReportStatsUseCaseTest {
    ...
    fun `an empty selection resolves to every account including archived`() = runTest {
        val entry = CapturingEntryRepository()
        val accounts = listOf(
            Account(currency = "BRL", id = 1, name = "open", type = AccountType.ASSET),
            Account(currency = "BRL", id = 2, name = "archived", type = AccountType.ASSET, isArchived = true),
        )
...
private class CapturingEntryRepository : StubEntryRepository() {
    ...
    override suspend fun scopeStats(scopeAccountIds: List<Long>, startDate: LocalDate, endDate: LocalDate): ScopeStats {
        ...
        return ScopeStats(brl(0.0), brl(0.0), brl(0.0), brl(0.0))
    }
