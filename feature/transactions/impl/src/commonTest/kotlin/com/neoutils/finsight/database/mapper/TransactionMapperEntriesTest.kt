...
class TransactionMapperEntriesTest {
    ...
    fun `mapper carries the hydrated entries onto the transaction`() {
        val account = Account(currency = "BRL", id = 1, name = "A", type = AccountType.ASSET)
        val entries = listOf(
            Entry(currency = "BRL", id = 1, transactionId = 7, account = account, amount = -5000),
            Entry(currency = "BRL", id = 2, transactionId = 7, account = Account(currency = "BRL", id = 10, name = "Food", type = AccountType.EXPENSE), amount = 5000),
        )
