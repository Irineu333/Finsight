...
- [x] 1.14 Adicionar seção curta no `CLAUDE.md` apontando para `.maestro/README.md` e regra "tela mudou ⇒ revisar testTag e flow"
...
- [x] 3.14 Disparar workflow no CI e confirmar 6 flows verdes (2 smoke + 4 transactions) — _validado pelo usuário via `workflow_dispatch` no GitHub Actions após o `Fix(E2E): uninstall stale e2e APK on cached AVD before installing`. Pin do `debug.keystore` segue como pré-requisito conhecido para promover a suíte a gate de PR (interação entre `~/.android/debug.keystore` regerado a cada run e o cache do AVD)._
...
- [x] 4.9 Rodar suíte completa e ajustar flaky; abrir issue para qualquer testTag faltante descoberto — _validado localmente em emulador (Galaxy_A54 API 17): 9/9 flows verdes (~6 min). Necessário aplicar `Modifier.testTagsAsResourceIdSemantics()` no `ExposedDropdownMenu` do `TargetSelector` (mesma classe do fix da fase 3 com `ModalBottomSheet`) — sem isso, testTags em `DropdownMenuItem` não viram `resource-id` no Android porque o popup escapa da raiz da Activity. Houve um flake único em `transactions / create expense` num primeiro re-run consecutivo (timeout em `accounts-root` durante o seed); passou em nova execução. iOS continua adiado conforme exceção das tasks 1.8/2.6/3.13._
