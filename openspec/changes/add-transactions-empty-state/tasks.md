...
- [ ] 1.1 Em `TransactionsUiState.kt`, declarar o `sealed interface ListState` com
...
- [ ] 1.2 Substituir o campo `transactions: Map<LocalDate, List<Transaction>>` por
  `listState: ListState = ListState.Loading` em `TransactionsUiState`.
- [ ] 1.3 Adicionar `data object ClearFilters : TransactionsAction()` em
  `TransactionsAction.kt`.

## 2. ViewModel

- [ ] 2.1 Em `TransactionsViewModel`, extrair a lista recortada para uma val local e derivar
...
- [ ] 2.2 Calcular `canClearFilters` a partir dos filtros **efetivos** — `target` e
  `installmentOnly` já neutralizados pelo escopo, mais `category`, `label` e `recurringOnly`
  (D3).
- [ ] 2.3 Tratar `TransactionsAction.ClearFilters` em `onAction` com
  `filters.value = TransactionsFilters()`, sem tocar `selectedYearMonth` nem `selectedScope`
  (D3/D4).
- [ ] 2.4 Confirmar que `initialValue = TransactionsUiState()` agora significa `Loading` e que
  nenhuma emissão posterior volta a esse estado (D6).

## 3. Strings

- [ ] 3.1 Adicionar em `core/resources/.../values/strings.xml`:
...
- [ ] 4.1 Criar o `private fun TransactionsEmptyState(...)` em `TransactionsScreen.kt`, no
...
- [ ] 4.2 Trocar o `uiState.transactions.forEach` por um `when (uiState.listState)`:
...
- [ ] 4.3 Verificar que `SummaryCard` e `FiltersRow` continuam emitidos em todos os estados
  (spec: controles do recorte permanecem visíveis).

## 5. Testes

- [ ] 5.1 Em `feature/transactions/impl/src/commonTest`, criar/estender o teste do ViewModel
...
- [ ] 5.2 Testar que `ClearFilters` devolve os filtros ao neutro preservando mês, escopo e
  `balanceOverview`.
- [ ] 5.3 Testar que um filtro neutralizado pelo escopo (alvo fora do escopo geral, parcelados
  sob o escopo de contas) não faz `canClearFilters` virar `true`.
- [ ] 5.4 Ajustar os testes existentes que leem `uiState.transactions` para o novo
  `listState`.

## 6. Verificação

- [ ] 6.1 `./gradlew :app:shared:testDebugUnitTest` verde.
- [ ] 6.2 `./gradlew :app:android:assembleDebug` compilando.
