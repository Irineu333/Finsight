...
- **Uma única primitiva de navegação, parametrizada por plataforma.** Selecionar um **item do seletor** normaliza a pilha (`popUpTo(start){inclusive=false}; launchSingleTop`); navegar para qualquer outro destino empilha normalmente com botão voltar. Mobile e desktop rodam a mesma lógica, diferindo só em (a) quais destinos são membros do seletor e (b) onde o seletor aparece. _(Nota: o desenho inicial usava `saveState`/`restoreState` para preservar pilha por seção; abandonado na fase 9 por incompatibilidade com rotas parametrizadas — ver design.md Decisão 1 e tasks 9.5/9.6.)_
...
- **BREAKING (interno)**: `HomeGraph` é dissolvido. Dashboard e Transactions viram graphs top-level (`dashboardGraph()`, `transactionsGraph()`) irmãos das demais features no `AppNavHost` achatado; `startDestination` passa a `DashboardGraph`. A troca de abas passa a `popUpTo(<start>){inclusive=false}; launchSingleTop` (sem `saveState`/`restoreState`).
...
- **Contrato de chrome e catálogo em `shell:api`.** `ChromeConfig`/`ChromeController`/`LocalChromeController`/`ChromeEffect` (encurtados de `HomeChrome*`, task 6.3) e o novo tipo de catálogo migram de `feature:home:api` para `feature:shell:api` (rename do módulo). Preserva o status quo (features já dependem de `home:api` para publicar chrome), sem descer para `:core`.
- **Botão voltar decidido por cada tela** via o helper `isWideWindow()`: telas host de seção ocultam o voltar no desktop (o rail é a navegação); sub-features sempre o exibem. Sem estado global de navegação. _(O desenho inicial centralizava a regra no `ChromeHost`; abandonado na fase 9 por flicker — ver design.md Decisão 6 e task 9.4.)_
...
- `navigation`: a topologia de abas muda de "`NavHost` com subgrafo `HomeGraph`" para "`NavHost` achatado com troca de seção normalizada (`popUpTo(start){inclusive=false}; launchSingleTop`)"; a "Chrome do Home" vira "Shell de navegação adaptativa unificada" (rail = todos os graphs, bottom bar = 2 abas, mesma primitiva); o contrato de chrome passa de `feature:home:api` para `feature:shell:api` (rename) e é encurtado para `Chrome*`.
