...
## 10. Casos de borda de arquivar/excluir (verificação pós-revisão)

Levantados por três agentes de exploração sobre o fluxo adjacente de arquivar/excluir cartão. O comportamento central já estava correto (excluir recusa com erro tipado e nunca arquiva como fallback; a decisão apresentação-vs-domínio tem dono único em `RetireAction`; seletores e listas ativas leem a query que exclui arquivados, com o `LedgerEntryWriter`/`closedLegBlockingChange` como rede final). As tarefas abaixo fecham os pontos frágeis encontrados.

- [x] 10.1 **Divergência UI × use case ("recorrência sem lançamentos"):** `CreditCardUi.retireAction` derivava só de `hasEntries`, mas `DeleteCreditCardUseCase` também recusa por `HAS_RECURRING` — um cartão com recorrência e sem entries no razão fazia a tela oferecer **Excluir** para o domínio recusar. Renomeia `CreditCardUi.hasMovement` → **`mustPreserve`** (nome honesto do parâmetro de `retireActionOf`) e passa a computá-lo como `hasEntries || hasRecurringForCreditCard` em `CreditCardsViewModel` e `InvoiceTransactionsViewModel` (ambos ganham `IRecurringRepository`; registros no `creditCardsModule` e fake no teste de caracterização atualizados).
- [x] 10.2 **`getCreditCardById` sempre reportava `isArchived = false`:** usava `dao.getCreditCardById` (entity plano, sem archival), assimétrico ao caminho `observe*` (que usa `WithArchival`). Substitui a query plana por `CreditCardDao.getCreditCardWithArchivalById` (mesmo `ALL_CREDIT_CARDS` do observe) e roteia `CreditCardRepository.getCreditCardById` por ela; remove a query plana, agora morta. Elimina o trap latente para futuros leitores de `isArchived` por id.
- [x] 10.3 **Guard de exclusão sem teste:** adiciona `DeleteCreditCardUseCaseTest` (recusa com lançamentos → `HAS_TRANSACTIONS`; recusa com recorrência → `HAS_RECURRING`, sem deletar; cartão sem movimento é deletado) e `ArchiveCreditCardUseCaseTest` (delega a conta a `ArchiveAccountUseCase` e nunca deleta; `NOT_FOUND` quando a conta sumiu). Trava que um refactor futuro não reintroduza fallback nem derrube os guards.
- [x] 10.4 **Decisão consciente — sem delegar `DeleteCreditCardUseCase` a um `DeleteAccountUseCase` compartilhado:** os guards de cartão chaveiam por `creditCardId` (recorrência) e a exclusão remove também a linha de fachada; forçar delegação esconderia uma decisão de negócio e quebraria o padrão "cada fachada é dona dos seus guards" (idêntico a `DeleteCategoryUseCase`/`DeleteAccountUseCaseImpl`). O risco de divergência das duas cópias do guard de entries é coberto por 10.3. Registrada como não-mudança deliberada.
- [x] 10.5 **Registrar pendência fora de escopo:** `AccountUi.hasMovement` tem a mesma divergência latente do 10.1 (exclusão de conta também recusa por `isDefault`/`hasRecurringForAccount`), mas o pedido foi restrito a cartão. Fica registrado para uma mudança própria no feature de accounts — não corrigido aqui.
- [x] 10.6 **Overflow da tela de faturas oferece desarquivar para cartão arquivado:** o item de retirar mostrava "Arquivar" mesmo num cartão já arquivado (beco sem saída). Agora `InvoiceTransactionsScreen` bifurca o item: cartão arquivado → **"Desarquivar"** (`credit_cards_unarchive`, `Icons.Default.Unarchive`) → `InvoiceTransactionsAction.Unarchive`; cartão ativo → Excluir/Arquivar conforme `retireAction` (inalterado). `InvoiceTransactionsViewModel` ganha `UnarchiveCreditCardUseCase` + `Crashlytics` e trata a ação resolvendo o cartão por id no momento do clique (`onLeft` → crashlytics), espelhando `ViewCreditCardViewModel`; sem confirmação (D8), a conta reaberta re-emite e a tela volta às ações de cartão ativo sozinha. `creditCardId` passa a `val`; DI e teste de caracterização atualizados. **Fora deste ajuste:** o item "Editar cartão" segue visível para arquivados (não escreve no razão) — não tocado.
