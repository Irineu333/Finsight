...
- [x] 1.1 Criar `EmptyStateMessage` em `:core:designsystem`
  ...
  ação abaixo (D5).
- [x] 1.2 Trocar o corpo do `TransactionsEmptyState` privado em `TransactionsScreen.kt` por uma
  chamada ao novo componente, sem mudança visual — a escolha de ícone, textos e do botão de
  limpar filtros continua na tela (D5).

## 2. Strings

- [x] 2.1 Adicionar em `core/resources/.../values/strings.xml` as seis famílias do D9:
...
- [x] 2.2 Adicionar as mesmas chaves em `values-en/strings.xml`.
- [x] 2.3 Confirmar que nenhuma tela cria chave própria para "Limpar filtros" — as três
  reaproveitam `transactions_empty_filter_action` (D9).

## 3. Contas

- [x] 3.1 Em `AccountsUiState`, declarar o `sealed interface ListState` com `EmptyAccount`,
...
- [x] 3.2 Substituir o campo `transactions` de `AccountsUiState.Content` por
  `listState: ListState`.
- [x] 3.3 Adicionar `data object ClearFilters : AccountsAction()`.
- [x] 3.4 Em `AccountsViewModel`, derivar `listState` pelo `when` do D2, usando
  `selectedAccountTransactions` (todos os meses) como recorte-raiz e `filteredTransactions`
  como lista visível.
- [x] 3.5 Calcular `canClearFilters` a partir de categoria, tipo e recorrentes — o mês fica de
  fora (D3).
- [x] 3.6 Tratar `AccountsAction.ClearFilters` em `onAction` devolvendo os filtros ao neutro,
  sem tocar `selectedMonth` nem `selectedAccountId` (D3).
- [x] 3.7 Em `AccountsScreen`, trocar `uiState.transactions.forEach` por um `when` sobre
...
- [x] 3.8 Verificar que pager, `AccountActions` e `FiltersRow` continuam emitidos nos dois
  estados vazios.

## 4. Cartões

- [x] 4.1 Em `CreditCardsUiState`, declarar o `ListState` com `EmptyInvoice`,
  `EmptyScope(canClearFilters)` e `Content(transactions)` e substituir o campo `transactions`
  de `Content` (D1).
- [x] 4.2 Adicionar `data object ClearFilters : CreditCardsAction()`.
- [x] 4.3 Em `CreditCardsViewModel`, derivar `listState` pelo `when` do D2 usando o resultado
  ...
  está.
- [x] 4.4 Calcular `canClearFilters` a partir de categoria, tipo, recorrentes e parcelados
  (D3), e tratar `ClearFilters` sem tocar `selectedCardId`.
- [x] 4.5 Em `CreditCardsScreen`, trocar `uiState.transactions.forEach` pelo `when` sobre
  `listState`, com `Outlined.CreditCard` no vazio de origem e `Outlined.FilterAltOff` no de
  recorte (D6/D8).

## 5. Faturas

- [x] 5.1 Em `InvoiceTransactionsUiState`, declarar o `ListState` com os **quatro** casos —
...
- [x] 5.2 Adicionar `data object ClearFilters : InvoiceTransactionsAction()`.
- [x] 5.3 Em `InvoiceTransactionsViewModel`, derivar `listState` pelo `when` do D2 usando
  `invoiceTransactions` (pré-filtro, da fatura selecionada) como recorte-raiz.
- [x] 5.4 Calcular `canClearFilters` (categoria, tipo, recorrentes, parcelados) e tratar
  `ClearFilters` sem tocar `selectedInvoiceIndex` (D3).
- [x] 5.5 Confirmar que `initialValue` agora significa `Loading` e que nenhuma emissão
  posterior — troca de fatura ou de filtro — volta a esse estado (D4).
- [x] 5.6 Em `InvoiceTransactionsScreen`, trocar `uiState.transactions.forEach` pelo `when`
...
- [x] 6.1 Em `feature/accounts/impl/src/commonTest`, testar o `AccountsViewModel`: conta sem
...
- [x] 6.2 Testar que `AccountsAction.ClearFilters` devolve os filtros ao neutro preservando mês
  e conta selecionada.
- [x] 6.3 Em `feature/creditcards/impl/src/commonTest`, testar o `CreditCardsViewModel`: fatura
...
- [x] 6.4 Testar o `InvoiceTransactionsViewModel`: estado inicial é `Loading`; fatura sem
  lançamentos → `EmptyInvoice`; filtro que corta tudo → `EmptyScope(canClearFilters = true)`;
  troca de fatura não volta a `Loading`.
- [x] 6.5 Ajustar os testes existentes que leem `uiState.transactions` nas três telas
  (incluindo `InvoiceTransactionsViewModelCharacterizationTest`) para o novo `listState`.

## 7. Verificação

- [x] 7.1 `./gradlew :app:shared:testDebugUnitTest` verde.
- [x] 7.2 `./gradlew :app:android:assembleDebug` compilando.
- [x] 7.3 Conferir nas telas: conta nova sem lançamentos, mês vazio de conta que movimenta,
