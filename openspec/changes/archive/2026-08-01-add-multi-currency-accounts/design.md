...
**Emenda, decidida na implementação: o requisito diz isso de si mesmo, e os seus cenários não fingem um usuário.** Ele estava redigido como fluxo — *"o usuário troca a moeda base de BRL para USD"* —, e é a delta spec que sobrevive ao arquivamento: quem chegasse depois leria uma capacidade que o produto não tem e procuraria a tela. Passa a afirmar, primeiro, que a troca **não é oferecida** (`IBaseCurrencyRepository` não tem setter, e a sua ausência é a decisão), e só então o que a change de fato deve — que trocá-la **permaneça derivação**, verificável hoje sobre o dado e sobre o acervo, sem exercer troca alguma. É o que `BaseCurrencySwitchDerivationTest` já prova em quatro casos (inversa, triangulação, *round-trip*, nenhuma linha gravada muda), e o que a ausência de valor convertido persistido sustenta.
...
Uma figura de dois termos (`R$ 100,00 + US$ 50,00`) não cabe em toda superfície. As de largura fixa ou de gramática própria — medidor de limite, rótulo de barra de progresso de orçamento — SHALL exibir apenas o termo na moeda base, com a marca de aproximação e a indicação de que há parcela não convertida.
...
**Segunda emenda, decidida na implementação: o `InstallmentCounter` também sai desta lista, e por razão distinta.** O exportado saiu por **poder** exibir os dois termos; este sai por **nunca receber** dois. A parcela é denominada pelo cartão em que ela é cobrada (D17: a moeda de uma conta é fixada na criação e não muda), então `InstallmentState` carrega a sua `currency` e a divisão é de um valor exato — nada foi reconciliado, nenhuma taxa participou, e a marca não lhe cabe. Degradar ali seria escolher um termo de uma lista que não existe, e teria feito o contador anunciar aproximação sobre um número que o sistema conhece perfeitamente. A lição generaliza e é o que a spec passa a dizer: onde a monomoeda é propriedade do **dado**, ela se expressa no tipo — o multitermo fica inexprimível naquela superfície — e não numa regra de exibição que escolhe um termo entre vários.
...
### D30 — As contas legadas são reetiquetadas pela **região do dispositivo**, uma vez e em silêncio
...
**Quem decide é a região, e o locale não é a região.** A primeira redação desta decisão dizia que a região decidia porque `NumberFormat.getCurrencyInstance()` resolve pelo país do locale. Isso é verdade sobre o *locale* e falso sobre o **dispositivo**: no iOS a região é um ajuste próprio, escolhido à parte do idioma, mas no Android o país vem da lista de idiomas, e ali não existe idioma sem país — escolher *English (United States)* é `en-US` seja qual for a moeda do usuário. A afirmação teria custado exatamente o que ela prometia impedir: um brasileiro que lê a interface em inglês teria **todo** o banco redenominado em dólar, irreversivelmente, por ler inglês.

Por isso a reetiquetagem — e só ela — pergunta a `DeviceRegion`, um tipo à parte de `localeCurrencyCode()`: no iOS é a região do sistema; no desktop, o ajuste de região do sistema operacional que a JVM expõe como país do locale; no Android, o país da operadora (SIM primeiro, rede depois), que é a única afirmação sobre **lugar** disponível sem depender da lista de idiomas. O locale continua decidindo o que ele sempre pôde decidir sozinho — a **pré-seleção** de um formulário, que o usuário vê e altera antes de qualquer escrita.

**O silêncio é uma resposta.** Um dispositivo que não sabe dizer onde está (um tablet sem SIM, um emulador) responde `null`, e `null` aqui significa o que sempre significou: não reetiquetar. Cair de volta no locale seria restaurar exatamente a leitura de que este tipo existe para desconfiar, e justamente nos aparelhos em que ela é menos confiável. O preço é assumido: esse usuário, se estiver de fato fora do Brasil, passa a ler `R$` sobre o histórico. É o mesmo caso do cohort fora do catálogo curado, e é preferível à alternativa — um dado permanentemente redenominado por um palpite.

**O falso positivo que resta, e ele é aceito:** um brasileiro que de fato **mora** em região estrangeira (SIM estrangeiro) tem as contas reetiquetadas para a moeda daquela região, sem aviso, e D12 impede desfazer. Duas coisas o estreitam: só um sinal de localização o dispara, e o catálogo curado barra moedas não oferecidas, que caem no caso silencioso de manter a constante.

Consequência aceita e registrada: para quem cair no falso positivo, o estado é **irreversível pelo app**. A alternativa considerada era perguntar uma vez aos usuários de região não-BRL, o que acertaria os dois casos ao preço de uma tela de migração; foi rejeitada em favor de zero atrito.
...
- **`netWorth()` não tem consumidor de produção.** A mudança de assinatura mais anunciada da change custa, hoje, apenas os ~25 fakes que implementam `IEntryRepository` inteiro. Ou é código morto, ou falta o consumidor — e o "saldo total" que o dashboard exibe vem de `CalculateBalanceUseCase(accountId = null)`, não dele.
  **Resolvido na 4.11: o membro sai de `IEntryRepository`.** A spec `ledger-reporting` exige que o patrimônio líquido seja derivável das entries, por moeda e sem as contas de conversão — ela não exige que o razão o exponha num membro de repositório. A leitura fica onde tem chamador de verdade: `EntryDao.netWorthCents()`, já agrupada por moeda e consumida pela paridade de migração (`LedgerFigures`) e pelos testes dos três cenários da spec. Dar-lhe consumidor de produção seria escopo novo disfarçado — o único candidato, o saldo total do dashboard, exibiria outra figura (`ASSET + LIABILITY` sem corte de mês, em vez de `ASSET` até o mês-alvo), e D6 já adia expor resultado cambial. Quando a consolidação lhe der consumidor, reintroduzir um membro de uma linha é trivial; carregar assinatura morta em 21 fakes até lá não é.
