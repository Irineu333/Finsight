...
- [x] 1.1 Adicionar em `EntryDao` a consulta que agrupa as entries de uma dimensão por
      `substr(o.date, 1, 7)` e por moeda, com a projeção correspondente (mês, moeda, total),
      filtrando pelo corte superior (D11)
- [x] 1.2 Declarar o membro em `IEntryRepository` no vocabulário do razão — dimensão e mês,
      ...
      derivado de relógio
- [x] 1.3 Implementar em `EntryRepository`, convertendo centavos pela mesma via das demais
      leituras, sem sinal próprio
- [x] 1.4 Atualizar os ~26 fakes de `IEntryRepository` nos testes; os que não exercitam a
      série lançam `NotImplementedError`, como já fazem para outras leituras
- [x] 1.5 Teste de razão: a série de uma dimensão em duas moedas devolve uma linha por
      (mês, moeda), e mês sem entry não aparece
- [x] 1.6 Teste de razão: o total de um mês pela série coincide com
...
- [x] 2.1 Definir em `feature/categories/api` o modelo de resultado do panorama: figura do
      mês corrente, figuras da janela (média e total) com o número de meses, variação, e a
      variante de estado da categoria
- [x] 2.2 Declarar `CalculateCategoryOverviewUseCase` em `feature/categories/api`
- [x] 2.3 Ler a série com corte superior no **mês corrente**, de modo que nenhum lançamento
      futuro alcance figura alguma (D11)
- [x] 2.4 Implementar em `feature/categories/impl` o cálculo da janela:
...
- [x] 2.5 Somar e dividir os meses da janela **por moeda**, antes de qualquer conversão, e
      consolidar cada figura uma única vez com `on` = último dia da janela
- [x] 2.6 Consolidar a figura do mês corrente com `on` = último dia do mês corrente
- [x] 2.7 Calcular a variação por `comparativeMagnitudes` sobre `{mês corrente, média}` com
...
- [x] 2.8 Resolver a variante de estado: arquivada, sem lançamento algum, sem mês fechado com
      lançamento, ou ativa
- [x] 2.9 Para a categoria arquivada, produzir o total histórico e o intervalo de datas, do
      primeiro ao **último lançamento** — nunca a data do arquivamento, que não existe (D10)
- [x] 2.10 Registrar o caso de uso no módulo Koin de categories
- [x] 2.11 Testes: janela cheia (12), janela encurtada (categoria jovem), mês sem lançamento
      contando como zero no divisor, e `média × meses = total` exato
- [x] 2.12 Testes: o mês corrente não entra na janela; um mês em duas moedas não é reduzido
      pelo razão
- [x] 2.13 Testes: cada um dos três casos de ausência de variação, e nenhum deles produzindo
      `0%`
- [x] 2.14 Testes: cada variante de estado devolve a figura de destaque correta
- [x] 2.15 Testes de lançamento futuro: parcelas em meses posteriores não entram no mês
...
- [x] 4.1 Adicionar `filterCategoryId: Long?` a `TransactionsRoute`, seguindo a nomenclatura
      de `filterLabel`/`filterTarget`
- [x] 4.2 Propagar o parâmetro de `transactionsGraph()` até `TransactionsScreen` e ao
      `ViewModel`, pela mesma via dos filtros já existentes
- [x] 4.3 No `TransactionsViewModel`, resolver o id para `SpendingSubject.Categorized` como
      estado inicial de `TransactionsFilters.subject`, aceitando categoria arquivada
- [x] 4.4 Um id que não resolve para categoria alguma deixa a lista no estado neutro, sem
      recorte e sem lista vazia
- [x] 4.5 Teste: a lista abre recortada e o controle exibe a categoria como selecionada
- [x] 4.6 Teste: escolher o estado neutro desfaz o recorte inicial
- [x] 4.7 Teste: categoria arquivada como valor inicial aparece selecionada mesmo não sendo
      oferecida no menu
- [x] 4.8 Teste: id que não resolve abre no estado neutro

## 5. Modal de categoria (D6, D7)

- [x] 5.1 Remover `MonthSelector`, `selectedYearMonth` e as ações `NextMonth`/`PreviousMonth`
      de `ViewCategoryAction`, `ViewCategoryUiState` e `ViewCategoryViewModel`
- [x] 5.2 Reescrever `ViewCategoryUiState.Content` sobre o resultado do caso de uso, com a
      variante de estado explícita
- [x] 5.3 Fazer o `ViewModel` observar o caso de uso, mantendo
