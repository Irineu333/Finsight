...
- [~] 6.3 Remover entidades/colunas legadas (`Transaction.Type`, `Target`, `Operation.Kind`) após confirmação de paridade <!-- FEITO: signedImpact(), Operation.Kind (col. dropada) e Transaction.Target (derivado de creditCard). RESTA Transaction.Type: diferente dos demais, NÃO é derivável — é informação primária. Uma despesa sem categoria, uma receita sem categoria e um ajuste têm todos category=null numa conta; só o Type os distingue no nível da perna. Recuperá-lo exige ler as entries da operação (sinal + tipo da conta-contra), i.e. a UI lendo entries — o redesenho do task 1.4, não uma derivação/limpeza. -->
