...
- **[Gasto por categoria virado para o razão — a pedido]** `CalculateCategory{Spending,Income}UseCase` (dashboard) reescritos como `Σ entries` da conta da categoria (task 4.4, antes marcada como CAP). Estimativa inicial ("refactor de módulo grande") estava errada: ~5 arquivos, +15 LOC, sem mudança de Gradle (`categories/impl` já via `transactions/api`). Coberto por teste de paridade. Relatório e formas in-memory permanecem legados (CAP-7).
...
- **[CAP-1 · RESOLVIDO (parcial)] Gasto/receita por categoria no dashboard agora derivam do razão.** `CalculateCategory{Spending,Income}UseCase` viraram interface em `categories/api` + impl entry-based em `categories/impl` (`Σ balanceInMonth` da conta da categoria; sinal invertido para INCOME), com teste de paridade (`CalculateCategorySpendingUseCaseImplTest`). A regra **api ⊄ api** foi respeitada sem mover módulo — `categories/impl` já dependia de `transactions/api`. **Correção da estimativa anterior:** eu havia classificado isto como refactor de módulo grande; na verdade foram ~5 arquivos / +15 LOC. **RESTA:** `CalculateReportCategorySpendingUseCase` (relatório) é um use case separado ainda legado; e há um **N+1** (uma query por categoria) que poderia virar uma única query agrupada.
...
- **[CAP-7 · coexistência] Double-write com leitores metade-razão/metade-legado.** Já vêm do razão: fatura, saldo(ajuste), patrimônio, e gasto/receita por categoria no dashboard. Ainda legados: a forma in-memory do `CalculateBalance` (lista de transações), os breakdowns por `Transaction`, e o category-spending do **relatório**. São paridade-corretos por espelhamento; a paridade dos que continuam legados não é garantida por teste automatizado (verificada em device). Risco de divergência a vigiar enquanto durar a coexistência.
