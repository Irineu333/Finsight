...
- **[BLOQUEADOR CRÍTICO corrigido — gate holístico de review] Migração crashava em dados legados sujos.** `MIGRATION_7_8` (passo 7) derivava a conta real da perna e inseria em `entries.accountId NOT NULL`. Transações órfãs existem em devices reais: `DeleteAccountUseCase` só apaga a conta (FK `SET_NULL` deixa `transactions.accountId=NULL`) e `DeleteCreditCardUseCase` preserva as pernas de pagamento (cartão apagado → `creditCardId=NULL`). Uma única perna órfã abortava o upgrade inteiro (brick / crash-loop). Comprovado com SQLite pelo revisor. **Corrigido:** conta `EQUITY:'Conta removida'` semeada; o passo 7 roteia pernas de conta real nula via `COALESCE` — mantém `Σ=0` e o gasto por categoria, invisível a saldos/patrimônio. Coberto por teste de migração com órfãos (conta E cartão apagados). Sem esse fix, não iria para produção. (Corner LOW inócuo: uma perna órfã de cartão poderia carregar `invoiceId`; mas apagar cartão faz CASCADE nas suas faturas → `transactions.invoiceId` vira NULL, então não contribui a nenhum owed.)

- **[Atomicidade do double-write corrigida — pega em review do PR (Copilot, procede)].** `OperationRepository.createOperation` fazia três escritas independentes (insert operation → insert transactions → `writeEntries`) e `updateOperation` fazia update + `rewriteEntries` (delete legs + re-insert) **sem transação**. Cada chamada de DAO commitava sozinha, então uma falha no meio (linha de fachada categoria/cartão ausente → `ensureXAccount` lança, cancelamento de coroutine, erro de DB) deixava estado parcial: operação + transactions legadas gravadas mas sem entries (create), ou — pior — as entries antigas já deletadas e nenhuma nova (update). Isso quebra o invariante "operação+entries atômicas" e a paridade que a coexistência (CAP-7) depende. **Corrigido:** ambos envolvidos num único `database.useWriterConnection { it.immediateTransaction { … } }` (Room 2.8.4 KMP; DAOs chamados dentro reutilizam a conexão confinada e entram na mesma transação, então rollback total em falha). A leitura final `getOperationById` fica fora da transação (dados já commitados). O 3º comentário do Copilot (FK enforcement supostamente desligada fora das migrations) **não procede**: o compilador Room KMP gera `onOpen` com `PRAGMA foreign_keys=ON` reaplicado por conexão quando há `@ForeignKey` — enforcement está ativo em runtime.
