...
- **[CAP-1 · RESOLVIDO] Gasto/receita por categoria (dashboard E relatório) derivam do razão.**
  ...
  - Relatório: `CalculateReportCategorySpendingUseCase` reescrito sobre o razão preservando a semântica de perspectiva (conta vs cartão) via correlação `EXISTS` de perna irmã (`EntryDao.categoryTotalsWithSiblingLeg`), com teste de SQL real (`EntryCategoryQueryTest`). ~7 arquivos, médio — o único ponto não-trivial era a correlação.
