...
- **THEN** ele obtém `DashboardEntry` e `TransactionsEntry` do Koin e invoca `register(builder)` em cada um, sem importar nada de `dashboard:impl` ou `transactions:impl`
