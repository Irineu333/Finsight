...
- **`TransactionUi.amount` muda de tipo** → quebra de compilação, não de runtime: dois consumidores de produção (ambos em `ReportExportLayout`) e um de teste (`TransactionPerspectiveTest`), todos no repositório.
