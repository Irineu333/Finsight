...
**Fora de escopo, registrado**: `InvoiceTransactionsScreen`, `CreditCardsScreen` e `ReportViewerScreen` chamam `toTransactionUi()` sem perspectiva. É um defeito separado; não afeta o ajuste, que tem uma única perna monetária, mas passa a afetar a transferência, cuja política depende da perspectiva.
