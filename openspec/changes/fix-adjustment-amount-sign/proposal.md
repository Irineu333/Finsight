...
- **Mudança visual 3** — a **transferência sem perspectiva** deixa de exibir o `−` que hoje é composto à mão. Vale para as listas sem perspectiva — transações, dashboard e parcelamentos. As telas de cartão e o ramo de cartão do relatório passam a declarar a sua (D11), e ali uma transferência não ocorre: ela é `ASSET → ASSET`.
...
- A escolha da política sai dos `@Composable` e passa para quem produz a figura — `BalanceOverviewFactory`, `AccountsViewModel`, `InvoiceTransactionsViewModel` e `ReportViewerUiState.Stats` —, nas duas superfícies. Sem isso a change escreveria um requisito que ela mesma violaria em cinco dos oito sítios.
...
- Absorção dos demais sítios da mesma política, **sem mudança de comportamento**, porque já obedecem à regra: o `enum SignDisplay` de `SummaryCard` (18 usos — a implementação de referência, de onde o tipo novo nasce), o `private enum AccountSignDisplay` de `AccountCard`, o trio de `Boolean` do `SummaryRow` da fatura, quatro literais `"+${...}"`/`"-${...}"` em `ReportContextCard` e `ReportExportLayout`, e o `formatWithSign` de `ViewAdjustmentUiState` — este último não catalogado de início, encontrado pela varredura final.
