...
- [x] 14.5 Fluxo Maestro em `.maestro/flows/backup/reach.yaml`, **rodado e verde**:
  ...
  `en-US`, `nokeys`, IME presente, `show_ime_with_hard_keyboard = 0`. Havia um segundo emulador
  ligado, então o alvo foi fixado por serial no `adb`, no `installDebug` e no Maestro.
  Não é asserção de enfeite: com `backupModule` fora da agregação o fluxo fica **vermelho**, e
  volta a verde quando reposto.
  O fluxo cobre **a travessia**, não exportar nem recusar: pelo §5.1 do README, cabe em E2E só o
  que não se responde com o sistema desmontado. Exportar e escolher arquivo saem do app para o
  seletor do sistema, e a recusa tem 19 testes sobre arquivos reais em `:core:database` mais 9 no
  ViewModel. O que só o app montado responde é se as quatro peças que ligam a feature funcionam
  juntas — nenhuma falha numa compilação, e duas falham em silêncio.
  Suíte inteira no mesmo aparelho: 13/15 verdes. `dashboard_initial_state` e
  `dashboard_customization` falharam na suíte e passaram isolados — instabilidade.
  `creditcards_lifecycle` falha de forma determinística no `assertNotVisible` do botão de fechar
  fatura, **e falha igual no `main` (`1fa3f0aa4`)**: é anterior a esta mudança, verificado rodando
  o mesmo fluxo com o app das duas branches.
