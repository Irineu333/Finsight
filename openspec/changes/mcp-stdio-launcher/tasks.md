...
- [x] 4.3 `McpServerOff` (`jvmMain`): com o interruptor desligado o processo fala o protocolo por um `Server` próprio — nenhuma ferramenta registrada, `tools/list` vazio e **qualquer** `tools/call` recusado nomeando a seção, inclusive um nome que não existe (o SDK responderia *"tool not found"*, que é a afirmação falsa que `McpPermissionNotice` existe para evitar). Não passa por `McpSessionFactory` porque as três coisas que o montador decide — instruções, lista e chamada — mudam todas nesse caso. O vocabulário é o mesmo: `McpPermissionNotice` ganhou `WHAT_THIS_IS` e `THE_SECTION`, e as frases antigas são montadas a partir deles sem mudar um byte.
