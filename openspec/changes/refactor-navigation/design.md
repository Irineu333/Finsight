...
### 4b. Todo grafo de feature é um `navigation<>`, sem exceção

A regra derivada da premissa 2 seria "subgrafo só quando há destino interno" — `report` e `support` teriam `navigation<>`, e `accounts`, `budgets`, `categories`, `recurring`, `creditcards` seriam `composable<>` irmãos no grafo raiz. Foi rejeitada por consistência.

O `navigation<>` uniforme custa um `data object <Nome>Graph` por feature. Compra três coisas: um alvo estável de `popUpTo` que não nomeia uma tela; um lugar óbvio onde transições de destino e deep links vão morar quando existirem; e um `AppNavHost` cuja leitura não exige saber quais features têm destino interno. O `creditcards` é o caso que decide — três destinos externos e irmãos, sem hierarquia natural, mas que passam a ter uma raiz nomeada.

Onde o `<Nome>Graph` reside continua sendo decidido pela premissa 2, não pela simetria: `SupportGraph` e `ReportGraph` vão para a `api` porque o dashboard navega até eles; `AccountsGraph`, `BudgetsGraph`, `CategoriesGraph`, `RecurringGraph`, `CreditCardsGraph`, `DashboardGraph` e `TransactionsGraph` vivem no `impl`, ao lado da extension, porque quem navega até essas features mira a tela e nunca o grafo.
...
**Estado das abas** → A hipótese inicial era que o `NavHost` aninhado preservava o estado de cada aba gratuitamente, e que a troca de abas passaria a exigir `popUpTo(HomeGraph) { saveState = true }` + `restoreState = true`. Falso nos dois termos. O `HomeScreen` já fazia `popUpTo(HomeRoute.Dashboard)`, destruindo a aba de transações a cada volta — nunca houve preservação de scroll. E `saveState`/`restoreState` são **incompatíveis** com este app: o dashboard empilha `TransactionsRoute` (widgets de receita/despesa), então o `popUpTo(HomeGraph) { saveState = true }` salva a pilha `[DashboardGraph, DashboardRoute, TransactionsGraph, TransactionsRoute]` indexada pelo destino mais fundo removido (`savedState.first()`, ver `NavControllerImpl.executePopOperations`) — isto é, pelo `DashboardGraph`. O `restoreState` do toque na aba Dashboard então restaura a pilha inteira, cujo topo é a tela de transações, e o toque vira um no-op visível.

A troca de abas usa `popUpTo(DashboardRoute) { inclusive = false }` + `launchSingleTop = true`, e o `NavigationItem` aponta para a **rota de tela** (`DashboardRoute`, `TransactionsRoute()`), não para o subgrafo — o `launchSingleTop` compara o destino do topo, e apontar para o grafo empilharia um dashboard duplicado. Preservar scroll entre abas continua possível, mas exige antes que a tela de transações aberta pelo dashboard deixe de ser um destino da aba de transações.
