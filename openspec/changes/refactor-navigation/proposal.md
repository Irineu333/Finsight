...
O `feature:report` já implementa o desenho correto (`ReportsRoute` na `api`, `ReportRoute.Config`/`.Viewer` no `impl`, subgrafo `navigation<ReportsRoute>`). Esta mudança generaliza esse padrão e remove a duplicação estrutural que o dispatcher sustenta.
...
- **`NavHost` único.** O `NavHost` aninhado do `HomeScreen` é substituído por `navigation<HomeRoute>(startDestination = DashboardRoute)` no grafo raiz. O `Scaffold` (bottom bar + FAB) sobe para `App()` e sua visibilidade passa a ser derivada do destino atual (`destination.hierarchy.any { it.hasRoute<HomeRoute>() }`) combinada com o `HomeChromeConfig` — que permanece, pois `DashboardScreen` legitimamente esconde a chrome no modo `Editing`.
...
**Alterados:** `App.kt`, `AppNavHost.kt`, `HomeRoute.kt`, `BottomNavigationBar.kt`; os `impl` de `dashboard`, `transactions`, `recurring`, `creditcards` e `support` (substituição de `dispatch(...)` por `navigate(...)`); `core/model` perde dois `NavType`.
