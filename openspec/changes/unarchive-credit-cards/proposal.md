...
- **Novo `ViewCreditCardModal`.** Cartão não tem modal de detalhe hoje; nasce um, espelhando `ViewCategoryModal`. Exibe um detalhe **decente** do cartão (identidade — nome e ícone, **sem cor**, que o cartão não possui; atributos — limite, dia de fechamento, dia de vencimento; **quantidade de faturas**, já que o saldo de um arquivado é sempre zero) e é onde vive o botão **Desarquivar**. Sem botão inline na lista: toca-se na linha, abre o modal. Ao desarquivar com sucesso o modal se fecha sozinho.
...
- **`feature/creditcards/impl`** — `CreditCardRepository.unarchive(accountId)` → `AccountDao.reopen` (o `accountDao` já está injetado); novo `UnarchiveCreditCardUseCase` (recebe o `CreditCard`, mesma forma de `ArchiveCreditCardUseCase`, outra fiação — direto ao repo); rota interna `ArchivedCreditCardsRoute`; UI model plano `ArchivedCreditCardUi` + mapper `CreditCard.toArchivedUi()` (o domínio não cruza a fronteira ViewModel → UI); nova `ArchivedCreditCardsScreen` + ViewModel (observa `observeAllCreditCardsIncludingClosed()`, filtra arquivados, mapeia para `ArchivedCreditCardUi`) + um `ArchivedCreditCardCard` novo; novo `ViewCreditCardModal` (`AdaptiveModal`) + ViewModel/UiState/Action/Event (detalhe + desarquivar, quantidade de faturas via `IInvoiceRepository`); slot `actions`/overflow na topbar de `CreditCardsScreen`; `creditCardsGraph()` registra a nova tela; registros no `creditCardsModule` / `useCaseModules`.
...
- **`core/resources`** — novas strings: título da tela de arquivados, vazio da tela, item do overflow, botão desarquivar, indicador textual de arquivado no card, rótulo da quantidade de faturas. Reusa os rótulos de limite/fechamento/vencimento já existentes.
