...
- **`feature/creditcards/api`** — `ICreditCardRepository.unarchive(id)`.
- **`feature/creditcards/impl`** — `CreditCardRepository.unarchive` (resolve `accountId` → `AccountDao.reopen`); novo `UnarchiveCreditCardUseCase` (espelha `ArchiveCreditCardUseCase`); rota interna `ArchivedCreditCardsRoute`; nova `ArchivedCreditCardsScreen` + ViewModel (observa `observeAllCreditCardsIncludingClosed()` e filtra arquivados); novo `ViewCreditCardModal` + ViewModel (detalhe + desarquivar); overflow na topbar de `CreditCardsScreen` navegando para a nova rota; `NavGraphBuilder.creditCardsGraph()` registra a nova tela; registros no `creditCardsModule` / `useCaseModule`.
- **`core/resources`** — novas strings: título da tela de arquivados, vazio da tela, item do overflow, rótulos do detalhe do cartão (limite/fechamento/vencimento/saldo), botão desarquivar.
