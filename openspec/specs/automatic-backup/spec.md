...
O app guardando cópias do acervo por conta própria, a partir do momento em que o usuário liga isso:
os três gatilhos — a abertura depois de um prazo, a ação destrutiva prestes a acontecer, e a
atualização que vai reescrever o banco —, a regra única que decide se há o que capturar, e as duas
portas pelas quais a pessoa mesma põe uma cópia no destino. Governa também onde as cópias ficam e o
que cada destino deixa de cobrir, o vínculo com a pasta apontada e o que fazer quando ele cai, o
histórico lido do destino em vez de uma tabela que viajaria dentro do backup, a retenção, a troca de
pasta, e a obrigação de a tela dizer sempre quando foi o último backup que deu certo. O mecanismo de
capturar, verificar e substituir é de `database-snapshot`; a promessa sobre o arquivo que o usuário
exporta à mão é de `local-backup`. O que esta capacidade governa é o app agindo sem que ninguém
peça — e o limite disso: com o cofre desligado, nada é escrito por nenhum dos gatilhos.
