# balance-adjustment Specification

## Purpose

O ajuste de saldo — de uma conta ou de uma fatura — é **uma única operação**: declarar o valor que o alvo deveria ter, na data em que essa declaração vale. Não há "saldo inicial", "saldo final" e "saldo atual" como conceitos: há um calendário, e os pontos de entrada da interface diferem apenas pela data com que abrem. O valor de referência exibido é sempre o saldo na data selecionada, de modo que o número mostrado e o número gravado não possam divergir. No razão, cada ajuste é um **delta** datado — nunca um valor-alvo persistido —, editado no lugar quando o mesmo alvo é reajustado na mesma data e removido quando resulta em zero. A data diz **quando a correção ocorreu**; a fatura diz **onde ela se liquida** (`invoice-governs-date`), e o valor chega à fatura pela dimensão, nunca pela data. Consome as leituras datadas do razão (`ledger-reporting`) sobre o plano de contas (`chart-of-accounts`).
## Requirements
