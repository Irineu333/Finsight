# category-spending-overview Specification

## Purpose

O que o detalhe de uma categoria informa sobre o dinheiro dela: quais figuras exibe, sobre qual janela elas falam, como essa janela é declarada em texto, como o mês corrente se anuncia como parcial, contra o que a variação compara e como ela se expressa sem reivindicar as cores que o app reserva a receita e a despesa, o que cada estado da categoria (ativa, arquivada, sem lançamento) exibe no lugar da figura de destaque, e por onde o detalhe leva aos lançamentos daquela categoria. O detalhe não navega no tempo — isso pertence à lista de lançamentos, alcançada a partir dele (`uncategorized-transaction-filter`). Os números vêm do razão (`ledger-reporting`), como `Σ entries` da dimensão agrupadas por mês; a redução a uma única moeda vem do redutor único da consolidação (`currency-consolidation`).
