...
A **criação de uma fatura** como operação única parametrizada pelo mês de vencimento: quem pode
criar, qual status resulta, o que a recusa, e a derivação da janela a partir do cartão. Existe
porque criar uma fatura é **declarar que aquele ciclo existiu** — a única informação do conjunto
que nada deriva —, e nunca declarar quanto ela valia: a janela e o vencimento vêm do cartão, o
valor vem dos lançamentos ou do ajuste de saldo.
