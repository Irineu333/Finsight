# invoice-payment-editing Specification

## Purpose

O pagamento **parcial** de fatura como operação **corrigível no lugar**: o mesmo formulário que a cria também a corrige, distinguindo os dois modos apenas pelo que anuncia — o cabeçalho e o verbo do único botão —, com cartão, fatura, conta pagadora, valor, contrapartida e data como campos livres, porque nenhum deles é identidade da operação. A correção preserva a identidade da transação — as pernas são reescritas, a operação continua sendo a mesma — e o pagamento **entre moedas** entra pela mesma porta, completado com pernas de conversão pela fronteira de escrita (`balanced-ledger`). Na correção o modo é fixo, e dele decorre o conjunto de faturas oferecido: só as que aceitam pagamento parcial, de modo que uma `CLOSED` fique fora por construção. Abrir preserva o que a operação registra hoje; trocar cartão ou fatura recalcula. Do que já existe herda, sem regra própria, o congelamento da quitação e de qualquer operação sobre fatura `CLOSED` ou `PAID` (`invoice-settlement`), o congelamento de uma perna sobre conta arquivada (`account-lifecycle`) e o acesso ao formulário pelo ponto de entrada público da feature dona dele (`feature-entry-points`).
