# invoice-purchase-window Specification

## Purpose

A **janela de compra** de uma fatura: o intervalo `[abertura, fechamento)` no dia de fechamento
do cartão que decide quais datas pertencem a ela, sua derivação a partir do cartão e do mês de
vencimento — com dono único em `:core:model` —, suas bordas, e a projeção de uma data nela
preservando o dia. Existe porque uma fatura **não tem um mês**: ela abre num, fecha no seguinte
e vence num terceiro, de modo que o mês exibido no seletor não nomeia nenhuma data de compra.
