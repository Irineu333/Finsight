...
Padronizar a documentação de cada módulo (feature e core) via `README.md` na raiz do diretório, e manter um índice de módulos no `CLAUDE.md` da raiz do projeto. Cada README descreve responsabilidade, contratos públicos do `:api`, dependências e responsabilidades internas do `:impl`.
