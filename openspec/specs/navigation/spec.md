...
- **THEN** a bottom bar é ocultada e a tela é exibida em modo empilhado com botão voltar, permanecendo o botão de ação visível — com as ações da tela, ou com a ação universal se ela não publicar nenhuma

#### Scenario: Tela publica ContentOnly
- **WHEN** a tela em foco publica `ChromeConfig.ContentOnly` via `ChromeEffect`
- **THEN** o seletor (rail ou bottom bar) e o botão de ação são ocultados em ambos os form factors — `ContentOnly` é a supressão explícita de que fala `contextual-fab`
