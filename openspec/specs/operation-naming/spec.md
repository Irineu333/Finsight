# operation-naming Specification

## Purpose

Como uma operação é nomeada em qualquer superfície que a exiba. A precedência é uma só — **título > categoria > forma** —, e nenhuma superfície a inverte para si. Os dois primeiros elos têm dono único, que responde com a ausência quando a operação não tem nem um nem outro; o terceiro pertence à superfície que o exibe, porque só ela sabe o que já foi dito ao redor — uma lista nomeia a operação por inteiro, um cabeçalho que já anunciou a natureza precisa completá-la. Nomear uma ausência com um literal genérico de reserva está proibido: a superfície ou nomeia a operação pela sua forma, ou omite a linha. E onde a garantia é do domínio, o invariante é afirmado e falha alto em vez de ser mascarado — a falha é o que denuncia um caminho de escrita que passe ao largo do dono da regra.
