# recurring-confirmation Specification

## Purpose
O que o usuário pode ajustar ao confirmar um ciclo de recorrência, o que a confirmação
escreve na transação e o que ela deixa intacto no template.

Uma recorrência é um **modelo do ciclo**, não uma sentença sobre ele: o ciclo confirmado
pode divergir do template no valor, na data, no destino, no título e na categoria, e
nenhuma dessas divergências volta para o modelo. Quem quer mudar o modelo edita o modelo.
## Requirements
