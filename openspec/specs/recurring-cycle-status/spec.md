# recurring-cycle-status Specification

## Purpose

A partição de um mês de recorrentes em quatro estados de ciclo — pendente, a lançar, lançado e ignorado —, o seu dono único no domínio, e como a tela a apresenta: o que a lista lista (ciclos, não templates), a ordem das seções, as contagens, a dobra, a ordenação interna e o que cada tipo de linha afirma. O estado é derivado, nunca persistido, e o corte que separa pendente de a lançar é uma comparação de datas. A linha de um ciclo lançado é lida do razão (`ledger-reporting`); as figuras do mês que a acompanham têm dono próprio (`recurring-month-overview`), e as arquivadas, que não geram ciclo, vivem fora desta lista (`recurring-archive`).
