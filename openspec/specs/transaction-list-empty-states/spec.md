# transaction-list-empty-states Specification

## Purpose

O que as listas de lançamentos das telas de **contas**, **cartões** e **faturas** afirmam quando não há o que listar: as duas leituras do vazio em cada uma — o recorte-raiz não ter lançamento algum, ou nenhum caber no recorte atual —, a proibição de a tela de faturas afirmar vazio antes da primeira leitura, a permanência dos controles que produzem o recorte, e quando a ação de limpar filtros é oferecida. Generaliza para essas três telas as regras que `transactions-empty-state` fixa para a tela de transações; o vazio de *nenhum cartão cadastrado*, que ocupa a tela inteira, não é governado aqui.
