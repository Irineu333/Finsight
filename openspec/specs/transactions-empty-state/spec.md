# transactions-empty-state Specification

## Purpose

O que a tela de transações **afirma quando não há o que listar**: as duas leituras do vazio — não existir transação alguma, ou nenhuma caber no recorte atual —, a saída que cada uma oferece, a proibição de afirmar vazio antes da primeira leitura do repositório, e a permanência dos controles que produzem o recorte. Recorta apenas o lugar da lista; o escopo, o período e o resumo continuam governados por `transaction-scope`.
## Requirements
