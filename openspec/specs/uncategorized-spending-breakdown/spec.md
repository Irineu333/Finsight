# uncategorized-spending-breakdown Specification

## Purpose

A regra de apresentação do total sem classificação num detalhamento por categoria: que ele existe como linha, que participa do denominador junto com as categorias, onde é posicionado, quando é omitido, e que ele nunca é uma categoria. O número em si vem do razão (`ledger-reporting`), como a ausência de dimensão na conta nominal; a escala e a fatia vêm do redutor (`currency-consolidation`). O que esta capacidade governa é o que a tela e o documento fazem com ele.
## Requirements
