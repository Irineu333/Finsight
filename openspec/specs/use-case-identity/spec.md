# use-case-identity Specification

## Purpose

A forma canônica de um use case público. A operação é identificada pelo **id** do que ela opera, e essa é a forma que carrega a implementação, resolvendo a identidade no momento da execução. A forma que recebe o agregado é uma conveniência que delega, nunca uma segunda implementação. Nenhum valor padrão de parâmetro deriva de outro parâmetro agregado, e uma leitura sobre vários itens recebe identidades e devolve o resultado indexado por elas.
