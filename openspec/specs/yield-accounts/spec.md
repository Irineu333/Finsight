# yield-accounts Specification

## Purpose

A conta líquida que rende — PicPay, Nubank, Inter — e o que o app precisa para que o rendimento dela não desapareça num ajuste de saldo. Uma conta `ASSET` **declara** que rende, e essa declaração governa apenas afordância: se o caminho de lançamento é oferecido e se a linha aparece zerada. Ela não participa de soma alguma.

Lançar um rendimento é registrar uma receita comum do razão — `ASSET ← INCOME` —, nunca um ajuste com contrapartida de reconciliação: "rendeu quanto" e "o saldo é quanto" são perguntas distintas. A contrapartida carrega a dimensão de **uma** categoria de sistema, identificada por chave e criada sob demanda, que o usuário pode renomear e adotar como sua; é essa dimensão, e só ela, que separa rendimento de receita em toda leitura, de modo que `ledger-reporting` recebe uma identidade de dimensão e continua sem saber o que ela é (`ledger-module-boundary`).

Onde a separação aparece é assunto de quem exibe: `transaction-scope` define que a linha reparticiona a de entradas, e `money-display` define o rótulo que declara isso. A proteção da categoria enquanto alguma conta rende é mais um guard de `account-lifecycle`, condicional e reversível — nada aqui é imutável por decreto.
